...
bool ofApp::DFS()//DFS탐색을 하는 함수
{
	...
	trace = (int*)malloc(sizeof(int) * room_num);
	nodePointer w;
	int next;
	push(&top, 0);

	while (top) {
		if (top->item == room_num - 1) return true;
		next = pop(&top);
		if (!visited[next]) {
			trace[trace_index++] = next;
			visited[next] = 1;

			for (w = graph[next]; w; w = w->link)
				if (!visited[w->vertex])
					push(&top, w->vertex);
		}
	}
...
{
	int prev = 0, curr = 1;
	stackPointer prev, curr;
	int startX = 50, startY = 50;
	//TO DO 
	//DFS를 수행한 결과를 그린다. (3주차 내용)
	for (; curr <= trace_index; prev++, curr++) {
		switch (trace[curr]-trace[prev])
		{
		case WIDTH:
			ofDrawLine(startX, startY, startX, startY+100);
			startY += 100;
